...
def evaluate(candidate,review,assessment,request,snapshot,weights,global_blockers):
    blockers=list(global_blockers)
    if candidate['product']!=request['product'] or candidate['product']!=snapshot['product']: blockers.append('product mismatch')
    if any(x['snapshot_hash']!=request['snapshot_hash'] for x in (candidate,review,assessment)): blockers.append('snapshot hash mismatch')
    if review['candidate_id']!=candidate['candidate_id'] or assessment['candidate_id']!=candidate['candidate_id']: blockers.append('candidate binding mismatch')
    ...
    inputs={**assessment['scores'],'evidence_strength':strength}
